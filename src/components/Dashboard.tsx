...
export default function Dashboard({
  ...
  const handleGroupsLoaded = useCallback((groups: Group[]) => { setGroupsCache(groups); }, []);
  const handleFoldersLoaded = useCallback((folders: ChatFolder[]) => { setFoldersCache(folders); }, []);
  const handleMediaCacheUpdate = useCallback((groupId: string, entry: MediaCacheEntry) => {
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <main className="flex min-w-0 flex-1 flex-col overflow-hidden pb-16 md:pb-0">
          ...
          <div className="flex-1 overflow-y-auto">
                ...
                <div className="flex w-full max-w-lg flex-col items-center gap-6 text-center">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
                      Browse your Telegram groups and channels,<br />manage media and search messages.
                    </p>
                  </div>
                  ...
                  <div className="grid w-full grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-zinc-500">Browse media</p>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Channels</p>
                        <p className="mt-0.5 text-xs text-zinc-500">Browse media</p>
                      </div>
                    </button>
                  </div>
            ...
            {(activeMenu === "groups" || activeMenu === "channels") && !selectedGroup && (
              <GroupsGrid
                session={session}
                type={activeMenu === "channels" ? "channels" : "groups"}
                activeFolderId={activeMenu === "channels" ? activeFolderByType.channels : activeFolderByType.groups}
                onActiveFolderChange={(folderId) => {
                  setActiveFolderByType((prev) =>
                    activeMenu === "channels" ? { ...prev, channels: folderId } : { ...prev, groups: folderId }
                  );
                }}
