...
function InitialsAvatar({ initials, size = "sm" }: { initials: string; size?: "sm" | "md" }) {
  ...
  return (
    <div className={`flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 font-bold text-white shadow-sm ${cls}`}>
      {initials}
    </div>
  );
}
...
function AccountAvatar({ account }: { account: TelegramAccount }) {
  ...
  useEffect(() => {
    ...
    async function loadPhoto() {
      ...
      try {
        ...
        if (!res.ok) { if (!cancelled) setPhotoFailed(true); return; }
  ...
  if (photoUrl && !photoFailed)
    return <img src={photoUrl} alt={getDisplayName(account.user)} onError={() => setPhotoFailed(true)} className="h-8 w-8 shrink-0 rounded-full object-cover shadow-sm" />;
  return <InitialsAvatar initials={getInitials(account.user)} />;
}

export default function Header({ user, session, accounts, currentAccountId, onSwitchAccount, onAddAccount, onSignOut }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    let objectUrl: string | null = null;
    async function loadPhoto() {
      setPhotoFailed(false); setPhotoUrl(null);
      try {
        const res = await fetch("/api/telegram/profile-photo", {
        ...
        objectUrl = URL.createObjectURL(blob);
        if (!cancelled) setPhotoUrl(objectUrl);
      } catch { if (!cancelled) setPhotoFailed(true); }
    }
    void loadPhoto();
    return () => { cancelled = true; if (objectUrl) URL.revokeObjectURL(objectUrl); };
  }, [session]);
  ...
  const profileAvatar = photoUrl && !photoFailed
    ? <img src={photoUrl} alt={user.firstName ?? "Profile"} onError={() => setPhotoFailed(true)} className="h-8 w-8 shrink-0 rounded-full object-cover ring-2 ring-white/20" />
      ...
      <div className="relative" ref={menuRef}>
        ...
        >
          ...
          <div className="hidden flex-col items-start sm:flex">
            ...
            {user.username && (
              <span className="text-[11px] leading-tight text-zinc-400">@{user.username}</span>
            )}
          </div>
          <svg className={`text-zinc-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="absolute right-0 top-full z-[60] mt-2 w-64 overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-2xl shadow-zinc-200/60 dark:border-zinc-700/60 dark:bg-zinc-900 dark:shadow-black/40">
            ...
            <div className="flex items-center gap-3 border-b border-zinc-100 px-4 py-3.5 dark:border-zinc-800">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-zinc-900 dark:text-zinc-100">{user.firstName} {user.lastName}</p>
                {user.username && <p className="truncate text-xs text-zinc-500">@{user.username}</p>}
                {user.phone && <p className="truncate text-xs text-zinc-400">+{user.phone}</p>}
              </div>
            </div>
            ...
            {accounts.length > 0 && (
              <div className="border-b border-zinc-100 p-1.5 dark:border-zinc-800">
                <p className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Switch Account</p>
                  ...
                  {accounts.map((account) => {
                    ...
                    return (
                      <button
                        key={account.id}
                        type="button"
                        onClick={() => { setOpen(false); onSwitchAccount(account.id); }}
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] font-medium">{getDisplayName(account.user)}</p>
                          {account.user.username && <p className="truncate text-xs text-zinc-400">@{account.user.username}</p>}
                        </div>
